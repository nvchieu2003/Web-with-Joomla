...
//For a reason I don't understand, sometime, the Html.php isn't called so it generates an error.
//At this point, I'm convinced that trying to update that component with FOF for Joomla4 is an error.
//I'm beginning to hate that framework so much, it hurts.
//Anyway, If returnurl isn't set here, we initialize it to avoid an error.
...
<input type="hidden" name="invoicing_invoice_id" value="<?php echo $this->item->invoicing_invoice_id ?>" />
			...
			<div class="row">
				<div class="col col-8">
					<fieldset id="details"> 
						...
						<div class="mb-3 row">
							<label class="col"><?php echo \JText::_('INVOICING_ORDER_NUMBER')?></label>
							<div class="col"><?php echo $this->item->order_number; ?></div> 
						</div>
						<div class="mb-3 row">
							<label class="col"><?php echo \JText::_('INVOICING_INVOICE_NUMBER')?></label>
							<div class="col"><?php echo $this->item->invoice_number; ?></div> 
						</div>
						<div class="mb-3 row">
						...
						<div class="mb-3 row">
							<label class="col"><?php	echo \JText::_('INVOICING_COMMON_INVOICE_STATUS')?></label>
							<div class="col"><?php echo InvoicingHelperSelect::invoicestatus($this->item->status,'status',array('class'=>'form-select'))?></div>					
						</div>
						<div class="mb-3 row">
						...
						<div class="mb-3 row">
							<label class="col"><?php	echo \JText::_('INVOICING_COMMON_INVOICE_INTEGRATOR')?></label>
							<div class="col"><input class="form-control" type="text" id="generator" name="generator" value="<?php echo $this->escape($this->item->generator); ?>" size="40"/></div>					
						</div>
						<div class="mb-3 row">
							<label class="col"><?php	echo \JText::_('INVOICING_COMMON_INVOICE_INTEGRATOR_KEY')?></label>
							<div class="col"><input class="form-control" type="text" id="generator_key" name="generator_key" value="<?php echo $this->escape($this->item->generator_key); ?>" size="40"/></div>					
						</div>
						...
						<!-- <div class="mb-3 row">
							<div class="col" class="PDFAttachment">
							<input type="hidden" name="PDFAttachment" value=""/> 
							<input type="checkbox" name="PDFAttachment" value="1" checked="checked"/>
							<?php echo \JText::_('INVOICING_INVOICE_PDF_ATTACHMENT')?>
							</div>
							<div class="col"> </div>
						</div> -->
...
<?php
	...
	function addFValue(id,name,source_key,description,quantity,gross,tax,net) {
		...
		var data = '<tr id=\"item_'+item_i+'\" class=\"ui-state-default values\">\
						<td class=\'td-small\'>\
							<span id=\'empty\' class=\'btn btn-outline-success\'><span class=\"icon-move\"></span></span>\
	...
	jQ(function() {
		jQ(document).ready(function(){
			...
			if (is_array($this->item->items) && count($this->item->items) > 0) {
				foreach($this->item->items as $item) {
				  $script .= "addFValue('".$item->invoicing_invoice_item_id."',";
			...
			$script .= "
				...
				'start': function (event, ui) {
							ui.placeholder.html('<div colspan=\"6\"></div>');
					},
					axis:'y'});
		});";
		
		if ($this->item->status == "PAID") {
			$script .= "if (!confirm('".\JText::_('INVOICING_INVOICE_PAID_MESSAGE')."')){
				history.back(-1);
			}";
		}
		...
		jQ('#status').change(function() {
			...
			if (options == 'PENDING'){
				...
				if (options == 'PAID') {
					jQ('.PDFAttachment').css('display','block');
				}
				else {
					jQ('.PDFAttachment').css('display','none');
				}
